...
class ShowStudents extends Controller
{
    ...
    public function showStudents(Request $request)
    {
        if ($this->checkRole()) {
            ...
            $head = Head::where('user_id', $user_id)->get();
            ...
            if ($request->has('degree') && $request->has('training_program') && $request->has('year_of_admission')) {
                ...
                if (!empty($students[0])) {
                    ...
                    return redirect('/head/students/get');
            ...
            return view('head.showStudents', ['degrees' => $degrees, 'training_programs' => $training_programs, 'message' => $message, 'status' => $status]);
